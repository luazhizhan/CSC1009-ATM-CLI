        ...
        for (int i = 0; i < headers.length; i++) {
            ...
            switch (headers[i]) {
                case "#":
                    ...
                    // table += new String(new char[entries.get(i).y - toPad.length()]).replace('\0', fill) + toPad;
            ...
            for (int j = 0; j < headers.length; j++) {
                ...
                switch (j) {
                    case 0:
                        ...
                        // table += new String(new char[entries.get(j).y - toPad.length()]).replace('\0', fill) + toPad;
                        // table += String.format("%" + entries. + "s", "#");
                    break;
                    // Replace static strings with an array somewhere
                    case 1:
                        table += fixedLengthString(entries.get(j).y, accounts.get(i).getAccountNumber().substring(0, Math.min(entries.get(j).y, 10)));
                    break;
                    case 2:
                        table += fixedLengthString(entries.get(j).y, accounts.get(i).getAccountHolder());
                    break;
                    case 3:
                        table += fixedLengthString(entries.get(j).y, Double.toString(accounts.get(i).getAvailableBalance()));
                    break;
                    case 4:
                        table += fixedLengthString(entries.get(j).y, Double.toString(accounts.get(i).getTotalBalance()));
                    break;
                    default:
                        System.out.println("Invalid table argument.");
                        break;
